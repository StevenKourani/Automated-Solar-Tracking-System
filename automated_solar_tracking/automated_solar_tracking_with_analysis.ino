...
const unsigned long intervalSeconds = 2; //20 min
bool timerActive = true; // disable timer for demo

unsigned long startMillis = 0;
unsigned long endMillis = 0;
unsigned long maxMillis = 0;
unsigned long minMillis = 4293967295;
float averageExecTime = 0;
...
void loop() {
  ...
  // Find WCET, BCET, AVGET
  unsigned long exectime = endMillis - startMillis;
  if (exectime > maxMillis) {
    maxMillis = exectime;
  }
  if (exectime < minMillis) {
    minMillis = exectime;
  }

  count++;
  averageExecTime += (exectime - averageExecTime) / count;

  #if WCET_LOGGING
  Serial.print(F("WCET (ms): ")); Serial.println(maxMillis);
  Serial.print(F("BCET (ms): ")); Serial.println(minMillis);
  Serial.print(F("ACET (ms): ")); Serial.println(averageExecTime);
  #endif
